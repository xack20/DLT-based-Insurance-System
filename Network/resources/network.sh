...
# @Date:   2023-07-28 14:51:43
# @Last Modified by:   Zakaria Hossain Foysal
# @Last Modified time: 2024-07-04 00:57:21
#!/bin/bash
#
...
function networkUp() {
  ...
  if [ "${RESTORE}" == true ]; then
    echo "ORG1_VOLUME_LOCATION=${BACKUP_PATH}/peer_backups/peer0.org1" > ./compose/env/.restore.env
    echo "ORG2_VOLUME_LOCATION=${BACKUP_PATH}/peer_backups/peer0.org2" >> ./compose/env/.restore.env
    echo "ORDERER_VOLUME_LOCATION=${BACKUP_PATH}/peer_backups/orderer" >> ./compose/env/.restore.env

    echo "ORG1_COUCHDB_VOLUME_LOCATION=${BACKUP_PATH}/couchdb_backups/couchdb0" >> ./compose/env/.restore.env
    echo "ORG2_COUCHDB_VOLUME_LOCATION=${BACKUP_PATH}/couchdb_backups/couchdb1" >> ./compose/env/.restore.env

    ENV_FILE_PATH=${PWD}"/compose/env/.restore.env"
  fi

  DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} --env-file ${ENV_FILE_PATH} up -d 2>&1
...
function upgradeCC() {
  scripts/upgradeCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE

  if [ $? -ne 0 ]; then
    fatalln "Upgrading chaincode failed"
  fi
}
...
function networkDown() {
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG3_FILES} --env-file ${ENV_FILE_PATH} down --volumes --remove-orphans
...
if [ "$MODE" == "prereq" ]; then
  ...
  networkDown
  networkUp
elif [ "$MODE" == "deployCC" ] || [ "$MODE" == "upgradeCC" ]; then
